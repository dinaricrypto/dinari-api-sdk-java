...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { DinariInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DinariInvalidDataException("`$name` is not set")
            is JsonNull -> throw DinariInvalidDataException("`$name` is null")
            else -> throw DinariInvalidDataException("`$name` is invalid, received $this")
        }
